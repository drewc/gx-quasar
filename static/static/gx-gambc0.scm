...
(define (make-class-type id super slots name plist ctor)
  ...
  (cond
   ((find (lambda (klass) (not (type-descriptor? klass))) super)
    => (lambda (klass)
         (error "Illegal super class; not a type descriptor" (list (%%type? klass)
                                                                   (eq? (%%structure-length klass) 12)))))
   ((find (lambda (klass)
...
(define (unchecked-field-ref obj off)
  (%%structure-ref obj (%%fx+ off 1) #f #f))
(define (unchecked-field-set! obj off val)
  (%%structure-set! obj (%%fx+ off 1) val #f #f))
...
(define (slot-ref obj slot #!optional (E &slot-error))
  (&slot-e obj slot (lambda (off) (%%structure-ref obj (%%fx+ off 1) #f #f)) E))

(define (slot-set! obj slot val #!optional (E &slot-error))
  (&slot-e obj slot (lambda (off) (%%vector-set! obj (%%fx+ off 1) val #f #f)) E))
...
(define (type-descriptor-super-set! type super)
  (%%structure-set! type 4 super #f #f))
